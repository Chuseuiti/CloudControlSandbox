#
# Self-Adaptive Fuzzy Logic ALgorithm
...
file=open("simulation_results/rewards.csv","a",0)
file_fuzzy=open("simulation_results/fuzzyevolution.csv","a",0)
...
class fl_class:
	...
	def __init__(self):
		...
		self.variability=0
		self.file_q_table=open("simulation_results/qtable.txt",'a')
		self.last_action=[[[],[],[]],[]]
		...
		self.memory_antecedent['memory_md']=memory_md=fuzz.trimf(memory,[0,85,100])
	...
	#
	#	# CPU(3) x Memory(4) x Variability-Memory(2) x Connections(4) x Latency(4) x Status(2) x #Servers(2) x RAM(3)= 4608 
	#	# Learning Fuzzy Classifiers Systems has already solved this challlenge. The focus of this algorithm is to optimize the already selected rules by evolving the membership functions.
	#
	############################################################
	...
	def adjustment_ctrl(self,new_state,status,latency,servers, ram,memory):
		...
		if self.step==0:
			...
			temp=(300 - np.abs(85-memory)*(30 if memory>85 else 15))
			#ref=((memory-70)/20 if memory>70 else (memory-70)/30)#50
			ref=((memory-85)/10 if memory>85 else (memory-85)/15)#50
			state=self.last_action[1]
			if state !=new_state:
				print("Adjusting jumps")
				ref=-ref
			value_reward+=temp
			...
			print("Reward Memory: "+str(300 - (np.abs(85-memory)*(30 if memory>85 else 15)) ))
			print("################")
			...
			if 1:#self.step==2:
				...
				if state==0:
					...
					if len(self.q[state])<3 :
						...
						print("Random Search")
						if new_state!=4:
							self.array_ram_lo=[99+randint(-50,50),150+randint(-50,50),200+randint(-50,50)]
						else:
						...
						if (self.step % 100!=0):
							...
							self.array_ram_lo=[self.q[state][position_reward][0][0][0][0]+randint(int(-10)+int(50*ref),int(10)+int(50*ref)),self.q[state][position_reward][0][0][0][1]+randint(int(-10)+int(50*ref),int(10)+int(50*ref)),self.q[state][position_reward][0][0][0][2]+randint(int(-10)+int(50*ref),int(10)+int(50*ref))]
							...
							if self.increase>10:
								...
								self.array_ram_med=[self.q[1][position_reward][0][0][0][0],self.q[1][position_reward][0][0][0][1]+randint(-10+int(50*ref),10+int(50*ref)),self.q[1][position_reward][0][0][0][2]+randint(-10+int(50*ref),10+int(50*ref))]
					...
					if len(self.q[state])<3:
						...
						print("Random Search")
						if new_state!=4:
							self.array_ram_med=[200+randint(-50,50),250+randint(-50,50),300+randint(-50,50)]
						else:
							print("Random Search on Neutral")
						...
						if self.increase_state0<13 and self.increase_state0 % 3==1:
							print("correction state 1")
							self.array_ram_med=[self.q[state][position_reward][0][0][0][0]+randint(int(-10)+int(50*ref),int(10)+int(50*ref)),self.q[state][position_reward][0][0][0][1]+randint(int(-10)+int(50*ref),int(10)+int(50*ref)),self.q[state][position_reward][0][0][0][2]+randint(int(-10)+int(50*ref),int(10)+int(50*ref))]

						else:
							self.array_ram_med=[self.q[state][position_reward][0][0][0][0]+randint(0,int(100)),self.q[state][position_reward][0][0][0][1]+randint(0,int(100)),self.q[state][position_reward][0][0][0][2]+randint(int(0),int(100))]
						...
						if self.increase_state0>10:
								...
								self.array_ram_lo=[self.q[0][position_reward][0][0][0][0],self.q[0][position_reward][0][0][0][1]+randint(-10+int(50*ref),10+int(50*ref)),self.q[0][position_reward][0][0][0][2]+randint(-10+int(50*ref),10+int(50*ref))]
					...
					if len(self.q[state])<3:
						...
						print("Random Search")
						if new_state!=4:
							self.array_ram_hi=[300+randint(-50,50),350+randint(-50,50),400+randint(-50,0)]	
						else:
							print("Random Search on Neutral")
						...
						if (self.step % 100!=0):
							...
							self.array_ram_hi=[self.q[state][position_reward][0][0][0][0]+randint(int(-10)+int(50*ref),int(10)+int(50*ref)),self.q[state][position_reward][0][0][0][1]+randint(int(-10)+int(50*ref),int(10)+int(50*ref)),self.q[state][position_reward][0][0][0][2]+randint(int(-10)+int(50*ref),int(10)+int(50*ref))]
				...
				#Self adaptive consistency verification # I have to clean this code yet 
				if self.array_ram_lo[0]<100:
					self.array_ram_lo[0]=100

				if self.array_ram_lo[1]<100:
					self.array_ram_lo[1]=100
				if self.array_ram_hi[2]>400:
					self.array_ram_hi[2]=400
				if self.array_ram_hi[0]<100:
					self.array_ram_hi[0]=100
				if self.array_ram_med[0]<100:
					self.array_ram_med[0]=100
				if self.array_ram_hi[1]<100:
					self.array_ram_hi[1]=100
				if self.array_ram_med[1]<100:
					self.array_ram_med[1]=100
				if self.array_ram_lo[2]<=150:
					self.array_ram_lo[2]=150

				if (self.array_ram_med[0]-self.array_ram_lo[0])<20:
					self.array_ram_med[0]=self.array_ram_lo[0]+20
				if (self.array_ram_hi[0]-self.array_ram_med[0])<20:
					self.array_ram_hi[0]=self.array_ram_med[0]+20

				if (self.array_ram_hi[1]-self.array_ram_med[0])<20 and (self.array_ram_hi[0]-self.array_ram_med[0])==20:
					self.array_ram_hi[1]=self.array_ram_med[0]+20

				#Controlability LAWS
				if self.array_ram_med[0]> self.array_ram_lo[2]:
					print("Controlling distance between rules")
					diff=((self.array_ram_med[0]-self.array_ram_lo[2])+2)/2 	
			
					self.array_ram_lo[2]+=diff
					self.array_ram_med[0]-=diff-1

				if self.array_ram_hi[0]> self.array_ram_med[2]:
					print("Controlling distance between rules")
					diff=((self.array_ram_hi[0]-self.array_ram_med[2])+2)/2
					self.array_ram_med[2]+=diff
					self.array_ram_hi[0]-=diff-1
				...
				if self.array_ram_lo[2]>=self.array_ram_med[2]: 
					print("Correction2")
					#self.array_ram_lo[2]=self.array_ram_med[2]-10
					diff=((self.array_ram_lo[2]-self.array_ram_med[2]))/2
					self.array_ram_med[2]+=diff
					self.array_ram_lo[2]-=diff


				if self.array_ram_med[2]>=self.array_ram_hi[2]:
					print("Correction1")

					diff=((self.array_ram_med[2]-self.array_ram_hi[2]))/2
					self.array_ram_hi[2]+=diff
					self.array_ram_med[2]-=diff

				if self.array_ram_lo[0]>=self.array_ram_med[0]: 
					print("Correction4")


					diff=((self.array_ram_lo[0]-self.array_ram_med[0]))/2
					self.array_ram_med[0]+=diff
					self.array_ram_lo[0]-=diff

				if self.array_ram_med[0]>=self.array_ram_hi[0]:
					print("Correction3")


					diff=((self.array_ram_med[0]-self.array_ram_hi[0]))/2
					self.array_ram_hi[0]+=diff
					self.array_ram_med[0]-=diff


				if self.array_ram_lo[0]<0:
					self.array_ram_lo[0]=100

				if self.array_ram_lo[1]<0:
					self.array_ram_lo[1]=100
				if self.array_ram_hi[2]>400:
					self.array_ram_hi[2]=400
				if self.array_ram_hi[1]>400:
					self.array_ram_hi[1]=400
				if self.array_ram_hi[0]>400:
					self.array_ram_hi[0]=400
				if self.array_ram_med[2]>400:
					self.array_ram_med[2]=390
				if self.array_ram_med[1]>400:
					self.array_ram_med[1]=389
				if self.array_ram_med[0]>400:
					self.array_ram_med[0]=388
				if self.array_ram_lo[2]>400:
					self.array_ram_lo[2]=380
				if self.array_ram_lo[1]>400:
					self.array_ram_lo[1]=379
				if self.array_ram_lo[0]>400:
					self.array_ram_lo[0]=378
				if self.array_ram_hi[0]<100:
					self.array_ram_hi[0]=100
				if self.array_ram_med[0]<0:
					self.array_ram_med[0]=100
				if self.array_ram_hi[1]<100:
					self.array_ram_hi[1]=100
				if self.array_ram_med[1]<100:
					self.array_ram_med[1]=100
				if self.array_ram_lo[2]<=100:
					self.array_ram_lo[2]=105
				if self.array_ram_lo[1]<=100:
					self.array_ram_lo[1]=100
				if self.array_ram_lo[0]<=100:
					self.array_ram_lo[0]=100
				...
				if self.array_ram_lo[2]>=self.array_ram_med[2]: 
					print("Correction2")
					self.array_ram_med[2]=self.array_ram_lo[2]+5

				if self.array_ram_lo[0]>=self.array_ram_med[0]: 
					print("Correction4")
					self.array_ram_med[0]=self.array_ram_lo[0]+5
				if (self.array_ram_hi[0]-self.array_ram_med[0])<10 and (self.array_ram_hi[2]-self.array_ram_med[2])<10 and self.array_ram_hi[1]<self.array_ram_med[1] :
					self.array_ram_hi[1]=self.array_ram_med[1]+5
				...
				if self.array_ram_hi[0]>self.array_ram_hi[1]:
					self.array_ram_hi[0]=self.array_ram_hi[1]
				...
				try:
					print("1-Oup")
					print(ram_state)
					if self.array_ram_lo[0]>ram_state:
						self.array_ram_lo[0]=ram_state
					if self.array_ram_hi[2]<ram_state:
						print("Ouuups")
						print(ram_state)
						print(self.array_ram_hi[2])
						self.array_ram_hi[2]=ram_state+5
				except Exception,e:
					print(str(e))

				try:
					print("2-Oup")
					if self.array_ram_lo[0]>self.ram_output:
						self.array_ram_lo[0]=self.ram_output
					if self.array_ram_hi[2]<self.ram_output:
						print("Ouuups2")
						print(self.ram_output)
						print(self.array_ram_hi[2])
						self.array_ram_hi[2]=self.ram_output
				except Exception,e:
					print(str(e))
				...
				if self.array_ram_hi[2]>400:
					self.array_ram_hi[2]=400
				if self.array_ram_hi[1]>398:
					self.array_ram_hi[1]=398
				if self.array_ram_hi[0]>392:
					self.array_ram_hi[0]=392
				if self.array_ram_med[2]>=393:
					self.array_ram_med[2]=393
				if self.array_ram_med[1]>=389:
					self.array_ram_med[1]=389
				if self.array_ram_med[0]>=388:
					self.array_ram_med[0]=388
				if self.array_ram_lo[2]>=380:
					self.array_ram_lo[2]=380
				if self.array_ram_lo[1]>=379:
					self.array_ram_lo[1]=379
				if self.array_ram_lo[0]>=378:
					self.array_ram_lo[0]=378
				if self.array_ram_hi[0]<100:
					self.array_ram_hi[0]=100
				if self.array_ram_med[0]<0:
				...
				if self.array_ram_med[0]>self.array_ram_med[1]:
					self.array_ram_med[1]=self.array_ram_med[0]
				if self.array_ram_med[1]>self.array_ram_med[2]:
					self.array_ram_med[2]=self.array_ram_med[1]
				if self.array_ram_lo[1]>self.array_ram_lo[2]:
					self.array_ram_lo[1]=self.array_ram_lo[2]
				if self.array_ram_lo[0]>self.array_ram_lo[1]:
					self.array_ram_lo[1]=self.array_ram_lo[0]
	...
	def fl_ctrl(self,cpu1, memory1, cpu2, memory2,connections1, connections2, latency, status, servers_on, ram_state ):
		...
		try:	
			if self.ram_output!=0:
				last_ram_output=int(self.ram_output)
		...
		try:
			...
			if memory1 >90 and ram_state > self.array_ram_hi[0]:
				server_output=2
			elif memory1 <40 and ram_state< self.array_ram_lo[1]:
				server_output=1
