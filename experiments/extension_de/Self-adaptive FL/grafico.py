...
plt.figure(1)
plt.subplot(411)
plt.plot(df.iloc[0:1800,2])
n=len(df.iloc[18:1830,2])
rmse = np.linalg.norm(df.iloc[18:1830,2] - 85) / np.sqrt(n)
...
plt.subplot(412)
plt.plot(df.iloc[0:1800,-1])
print("Server 1: ",len(df.iloc[0:1717,-1][df.iloc[0:1717,-1]==1].dropna(axis=0)))

tmp=1
list_values=[]
num=0
flag=0
for value in df.iloc[0:1717,-1]:
...
plt.subplot(414)
plt.plot(df.iloc[0:1800,5],'orange')
plt.xlabel("Iteration")
